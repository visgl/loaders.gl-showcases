...
export const getTileDebugInfo = (tileHeader) => {
  return {
    ...
    "Screen Space Error":
      formatFloatNumber(tileHeader._screenSpaceError) || NO_DATA,
  };
};
 ...
 * @returns {void}
 */
const checkBoundingVolumes = (tile, tileWarnings) => {
  if (!tile.parent) {
    return;
  }
 ...
 */
const createBoundingSphereFromTileMbs = (mbs) =>
  new BoundingSphere([mbs[0], mbs[1], mbs[2]], mbs[3]);
/**
 * Generates OrientedBoundingBox from tile obb data
 * @param {array} obb
 * @returns {OrientedBoundingBox}
 */
const createBoundingBoxFromTileObb = (obb) => {
 ...
 */
const checkLOD = (tile, tileWarnings) => {
 ...
 * @returns {boolean}
 */
const isAllVerticesInsideBoundingVolume = (boundingVolume, positions) => {
 ...
 */
const createBoundingVolumeFromTile = (tile, boundingType) => {
  switch (boundingType) {
    case OBB: {
