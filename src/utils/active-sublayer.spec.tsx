...
describe("Active Sublayer", () => {
  ...
  it("IsLeaf() should be true only for leafs", () => {
    ...
    activeSublayer.sublayers.forEach(leaf => expect(leaf.isLeaf()).toBe(true))
  });
  ...
  it("onChildVisibilityChange() should update visibility according to children visibility", () => {
    ...
    expect(changedLeafs.findIndex(sublayer => sublayer.id === 0)).toBeGreaterThan(-1)
    expect(changedLeafs.findIndex(sublayer => sublayer.id === 1)).toBeGreaterThan(-1)
  });
});
